...
def test_starter_end_to_end_against_the_full_fixture_set(labelled_fixtures):
    ...
    assert report["rejected"] == 0, "prosecutor must never emit a schema-invalid or over-quota claim"

    # precision perfect: it never guesses wrong
    assert report["precision"] == 1.0
    # recall: high performance across all classes
